...
\begin{frame}
  ...
  \begin{tikzpicture}[node distance=2cm]
    ...
    \node (text2)   at ( 3, -3.2) [textbox] {over many \textbf{studies}};
    \uncover<2->{
    \node (bvslr)   at ( 0, -5) [highlight] {BVSLR};
    }
    \coordinate (z) at ( 0, -1);
    \draw [line] (improve) -- (z);
    \draw [arrow] (z) -| (meta);
...
\begin{frame}
  \frametitle{Broad overview of BVSLR}
  \begin{itemize}
    \item Learn the hyperparameters $( \bs\theta )$ from the data
    \item The posterior probability for SNP $i$ to be causal is
          \begin{empheq}[box=\mymathbox]{equation*}
            p(z_i = 1 \mid \bs\phi, \vx, \theta) {\color{hidden}\  = \sum_{\vz: z_i=1} p(\vz \mid \bs\phi, \vx, \bs\theta)}
          \end{empheq}
    \item The probability for a locus to be causal $=$ 1 $-$ probability of \emph{not} having a single causal SNP
          \begin{empheq}[box=\mymathbox]{equation*}
...
\begin{frame}
  ...
  {\center
    \vskip -1em
    \only<1-1>{
      \includegraphics[height=0.8\textheight]{figures/bvslr_meta_method_00.png}\\
    }
    \only<2-2>{
      \includegraphics[height=0.8\textheight]{figures/bvslr_meta_method_01.png}\\
    }
    \only<3-3>{
      \includegraphics[height=0.8\textheight]{figures/bvslr_meta_method_02.png}\\
    }
    \only<4-4>{
      \includegraphics[height=0.8\textheight]{figures/bvslr_meta_method_03.png}\\
    }
    \only<5-5>{
      \includegraphics[height=0.8\textheight]{figures/bvslr_meta_method_04.png}\\
    }
    \only<6-6>{
      \includegraphics[height=0.8\textheight]{figures/bvslr_meta_method_05.png}\\
    }
  }
\end{frame}
...
\begin{frame}
  \frametitle{Correlation vs coupling}
  \myfulltwocolumn{0.6}{T}
  {
    ...
    \begin{itemize1}
      ...
      \item Non-causal loci in correlation with multiple causal loci reduces precision of SNPTEST
    \end{itemize1}
  }
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize1}
    ...
    \item Assumes underlying data \textit{must} have sufficient causal and non-causal SNPs
  \end{itemize1}
\end{frame}
...
\begin{frame}
  \frametitle{Possible extensions of BVSLR}
  \begin{itemize1}
    \item Integrate additional data \eg ENCODE, DeepSEA, etc.
    \item Gene-environment interaction
    \item Dominant and recessive models
    \item Integration with GTEx
...
\begin{frame}[noframenumbering]
  \frametitle{Integration with GTEx}
  \myreference{Gamazon \etal, Nat. Genet. 2015}
  {\center
    \vskip -2em
    \includegraphics[height=0.8\textheight]{figures/PrediXcan.jpg}\\
  }
\end{frame}
