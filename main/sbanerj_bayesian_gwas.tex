...
\title{Bayesian multivariate metaanalysis in GWAS \\ (BammGWAS)}
...
\begin{frame}
  ...
  \begin{tikzpicture}[node distance=2cm]
    ...
    \node (text2)   at ( 3, -3.2) [textbox] {over many \textbf{studies}};
    \uncover<2->{
    \node (bvslr)   at ( 0, -5) [highlight] {BammGWAS};
    }
    \coordinate (z) at ( 0, -1);
    \draw [line] (improve) -- (z);
    \draw [arrow] (z) -| (meta);
...
\begin{frame}
  ...
  {
    \begin{tikzpicture}
      \node (a) at (0,0) [highlight] {\textbf{BammGWAS}};
    \end{tikzpicture}
  }{
    Banerjee and S\"oding, \textit{manuscript prepared} 2017\\
  }
...
\begin{frame}
  \frametitle{Sparsity in BVSR (BIMBAM) / BVSLR (BammGWAS)}
...
\begin{frame}
  \frametitle{Priors in BVSLR (BammGWAS)}
  \begin{empheq}[box=\mymathbox]{equation*}
...
\begin{frame}
  \frametitle{Inference of causality in BVSLR (BammGWAS)}
...
\begin{frame}
  ...
  \uncover<3->{
  \begin{itemize1}
    \item Applied BammGWAS on these 45 loci, selecting 400 SNPs at each locus.
  \end{itemize1}
  }
\end{frame}

\begin{frame}
  \frametitle{BammGWAS predictions}
  \only<1-1>
  {
  \vskip -1em
...
\begin{frame}
  \frametitle{Top BammGWAS predicted loci (not discovered in CardiogramPlusC4D)}
  \vskip -2em
  {\center \scriptsize
...
\begin{frame}
  ...
  {\center
   ...
   \begin{itemize1}
     \item Applied BammGWAS on these 200 loci, selecting 400 SNPs at each locus.
   \end{itemize1}
  }
  \only<2->
  {\center
